...
```{r}
mean(my_flights$gain) 
mean(my_flights$gain, na.rm = T)

my_flights %>% pull(gain) %>% mean(na.rm = T)
...
my_flights %>% 
  ...
  summarise(
    ...
    min_airtime = min(air_time, na.rm = T),
    max_airtime = max(air_time, na.rm = T)
  )
...
```{r}
my_flights %>% group_by(origin) %>% slice_max(dep_delay, n = 1) %>% select(origin, dep_delay, everything())
...
```{r}
starwars2 <- starwars
starwars2 %>% count(homeworld, sort = TRUE)
...
```{r}
starwars2 %>% filter(eye_color == 'brown', is.na(height))
...
```{r}
starwars2 %>% group_by(eye_color) %>% summarise(avg_height = mean(height, na.rm = T))
...
```{r}
starwars %>% group_by(sex == 'male') %>% slice_max(height)
